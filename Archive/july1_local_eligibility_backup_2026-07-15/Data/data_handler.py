...
def load_gt_data(args):
    ...
    #Load in second dataset optionally
    if args["simulation"]["dataset2"]:
        mat2 = loadmat(repo_root / args["paths"]["data2"],variable_names=["all_data"],squeeze_me=True,struct_as_record=False)
        all_data2 = mat2["all_data"]
    ...
    for k,cell in enumerate(args["simulation"]["cell_targets"]):
        ...
        if cell <= 220:
            ...
            timestamps = all_data2[cell-220-1].passive_tar1_timestamps
            if args["simulation"]["data_target"] == "peak":
                if all_data2[cell-220-1].tuning_type == 'contra-tuned':
                    angle = 0
                elif all_data2[cell-220-1].tuning_type == '45°-tuned':
                    angle = 1
                elif all_data2[cell-220-1].tuning_type == 'center-tuned':
                    angle = 2
                else:
                    angle = 3
