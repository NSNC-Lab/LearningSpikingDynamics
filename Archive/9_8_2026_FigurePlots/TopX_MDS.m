...
%Do selection
Choose_by = 'SPIKE';
[selection,cell_identity] = calculate_selction_topX(Choose_by, sim_object, data_object, top_perc);

%Calculate MDS positions from joint Projection
dis_mat = calc_dis(Distance_measure,data_object,sim_object,selection,cell_identity); 
...
ax(1) = nexttile;
scatter(Y(1:length(selection),1),Y(1:length(selection),2),20,fr_data,'filled');
title('Firing rate')
...
function dissimilarity_matrix = calc_dis(Distance_measure,data_object,sim_object,selection,cell_identity)
    ...
    if strcmp(Distance_measure,'RMSE')
        ...
        for k = 1:length(selection)
            ...
            for z = 1:10
                spikes{z+10} = find(sim_object.output(selection(k),z,cell_identity(k),:))/10000;
            end
            data_times = [];
            for m = 1:10
        ...
        dissimilarity_matrix = zeros([length(selection)*2,length(selection)*2]);

        for k = 1:length(selection)*2
            for z  = 1:length(selection)*2
...
function [selection,cell_identity] = calculate_selction_topX(choose_by, sim_object, data_object, top_perc)
