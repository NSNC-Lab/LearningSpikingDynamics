...
def save(states,path="simulation_output.mat",checkpoint=False):
    ...
    data.update({'sse_losses':np.asarray(states['neurons']['BookKeeping']['sse_loss'].cpu(), np.float32), 'cv_losses':np.asarray(states['neurons']['BookKeeping']['cv_loss'].cpu(), np.float32), 'rate_losses':np.asarray(states['neurons']['BookKeeping']['rate_loss'].cpu(), np.float32), 'fused_losses':np.asarray(states['neurons']['BookKeeping']['fused_loss'].cpu(), np.float32)})
