...
def handle_loss(args, states, gt_data, timestep,epoch):
    ...
    if (timestep + 1) % args['simulation']['PSTH_granularity'] == 0 and timestep > 0:
        loss = calculate_loss(states, gt_data['psth_holder'], args, timestep, epoch)
    ...
    if timestep == args['simulation']['sim_len'] - 1:
        ...
        states = update_grad_CV_and_rate(states, loss_cv['gradient'], gt_data['psth_holder'], args, timestep,args["simulation"]["PSTH_granularity"],epoch)

    return states


def calculate_loss(states,gt_data,args,timestep,epoch):
    ...
    states["neurons"]['BookKeeping']['sse_loss'][:, :, epoch] += loss
    ...
    for k in range(args['simulation']['batch_size']):
        for m in range(len(args['simulation']['cell_targets'])):
            ...
            if len(isi_holder_sim) <= 1: #The idea behind this is that if there is nothing to go off of (aka no spikes) don't move the gradient... The other loss should rebound things (assuming that the refractory parameters are well enough constrained)
                gradient[k,m] = 0
                sim_cv = 0 #Default if sim is undefined.
            ...
            states["neurons"]['BookKeeping']['cv_loss'][k, m, epoch] = args["simulation"]["lamda2"] *(sim_cv - data_cv)**2
...
def update_grad(states,gradient):
    ...
    states["neurons"]["Learnable"]["ron"]["abs_ref_accum_rate"] += states["neurons"]["Learnable"]["ron"]["abs_ref_accum"]
    ...
    states["neurons"]["Learnable"]["ron"]["rel_ref_a_accum_rate"] += states["neurons"]["Learnable"]["ron"]["rel_ref_a_accum"]
    ...
    states["neurons"]["Learnable"]["ron"]["rel_ref_b_accum_rate"] += states["neurons"]["Learnable"]["ron"]["rel_ref_b_accum"]
    ...
    states["neurons"]["Learnable"]["ron"]["rel_ref_c_accum_rate"] += states["neurons"]["Learnable"]["ron"]["rel_ref_c_accum"]
    states["neurons"]["Learnable"]["ron"]["rel_ref_c_accum"].zero_()

    return states

def update_grad_CV_and_rate(states,cv_loss_gradient, gt_data, args, timestep, granularity,epoch):
    ...
    loss_gradient_rate = 2 * args["simulation"]["lamda1"] * (sim_rate-target_rate) / exposure

    states["neurons"]['BookKeeping']['rate_loss'][:, :, epoch] = args["simulation"]["lamda1"] * (sim_rate-target_rate)**2 / exposure

    #Adjust the weighting on the CV-specific loss
    cv_loss_gradient = args["simulation"]["lamda2"] * cv_loss_gradient

    states["neurons"]['BookKeeping']['fused_loss'][:, :, epoch] = states["neurons"]['BookKeeping']['rate_loss'][:, :, epoch] + states["neurons"]['BookKeeping']['cv_loss'][:, :, epoch] + states['neurons']['BookKeeping']['sse_loss'][:, :, epoch]
    ...
    for k in list(states['synapses']['Static'].keys()): 
        if k.rsplit("_")[1] == "ron":
            states["synapses"]["Learnable"][k]["gSYN_grad"] += loss_gradient_rate*states["synapses"]["Learnable"][k]["gSYN_accum_rate"]
            states["synapses"]["Learnable"][k]["gSYN_grad"] += cv_loss_gradient*states["synapses"]["Learnable"][k]["gSYN_accum_rate"]
        else:
            states["synapses"]["Learnable"][k]["gSYN_grad"] += loss_gradient_rate*states["synapses"]["Learnable"][k]["gSYN_accum_rate"]
            states["synapses"]["Learnable"][k]["gSYN_grad"] += cv_loss_gradient*states["synapses"]["Learnable"][k]["gSYN_accum_rate"]
        states["synapses"]["Learnable"][k]["gSYN_accum_rate"].zero_()

    states["neurons"]["Learnable"]["ron"]["output_ad_grad"] += loss_gradient_rate*states["neurons"]["Learnable"]["ron"]["output_ad_accum_rate"]
    states["neurons"]["Learnable"]["ron"]["output_ad_grad"] += cv_loss_gradient*states["neurons"]["Learnable"]["ron"]["output_ad_accum_rate"]
    states["neurons"]["Learnable"]["ron"]["output_ad_accum_rate"].zero_()

    states["neurons"]["Learnable"]["STRF_alpha_grad"] += loss_gradient_rate*states["neurons"]["Learnable"]["STRF_alpha_accum_rate"]
    states["neurons"]["Learnable"]["STRF_alpha_grad"] += cv_loss_gradient*states["neurons"]["Learnable"]["STRF_alpha_accum_rate"]
    states["neurons"]["Learnable"]["STRF_alpha_accum_rate"].zero_()

    states["neurons"]["Learnable"]["STRF_gain_grad"] += loss_gradient_rate*states["neurons"]["Learnable"]["STRF_gain_accum_rate"]
    states["neurons"]["Learnable"]["STRF_gain_grad"] += cv_loss_gradient*states["neurons"]["Learnable"]["STRF_gain_accum_rate"]
    states["neurons"]["Learnable"]["STRF_gain_accum_rate"].zero_()

    states["neurons"]["Learnable"]["ron"]["abs_ref_grad"] += loss_gradient_rate*states["neurons"]["Learnable"]["ron"]["abs_ref_accum_rate"]
    states["neurons"]["Learnable"]["ron"]["abs_ref_grad"] += cv_loss_gradient*states["neurons"]["Learnable"]["ron"]["abs_ref_accum_rate"]
    states["neurons"]["Learnable"]["ron"]["abs_ref_accum_rate"].zero_()

    states["neurons"]["Learnable"]["ron"]["rel_ref_a_grad"] += loss_gradient_rate*states["neurons"]["Learnable"]["ron"]["rel_ref_a_accum_rate"]
    states["neurons"]["Learnable"]["ron"]["rel_ref_a_grad"] += cv_loss_gradient*states["neurons"]["Learnable"]["ron"]["rel_ref_a_accum_rate"]
    states["neurons"]["Learnable"]["ron"]["rel_ref_a_accum_rate"].zero_()

    states["neurons"]["Learnable"]["ron"]["rel_ref_b_grad"] += loss_gradient_rate*states["neurons"]["Learnable"]["ron"]["rel_ref_b_accum_rate"]
    states["neurons"]["Learnable"]["ron"]["rel_ref_b_grad"] += cv_loss_gradient*states["neurons"]["Learnable"]["ron"]["rel_ref_b_accum_rate"]
    states["neurons"]["Learnable"]["ron"]["rel_ref_b_accum_rate"].zero_()

    states["neurons"]["Learnable"]["ron"]["rel_ref_c_grad"] += loss_gradient_rate*states["neurons"]["Learnable"]["ron"]["rel_ref_c_accum_rate"]
    states["neurons"]["Learnable"]["ron"]["rel_ref_c_grad"] += cv_loss_gradient*states["neurons"]["Learnable"]["ron"]["rel_ref_c_accum_rate"]
    states["neurons"]["Learnable"]["ron"]["rel_ref_c_accum_rate"].zero_()
