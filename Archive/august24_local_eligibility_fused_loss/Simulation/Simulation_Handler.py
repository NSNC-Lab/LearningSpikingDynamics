...
def run_optimization(args, params, gt_data):
    ...
    for epoch in range(args['simulation']['epochs']):
        with torch.no_grad():
            ...
            print(f"Epoch {start_epoch+epoch} -- Average SSE Loss: {torch.mean(torch.mean(states['neurons']['BookKeeping']['sse_loss'][:,:,epoch])).cpu():.2f} -- Average CV Loss: {torch.mean(torch.mean(states['neurons']['BookKeeping']['cv_loss'][:,:,epoch])).cpu():.2f} -- Average Rate Loss: {torch.mean(torch.mean(states['neurons']['BookKeeping']['rate_loss'][:,:,epoch])).cpu():.2f} -- Average Fused Loss: {torch.mean(torch.mean(states['neurons']['BookKeeping']['fused_loss'][:,:,epoch])).cpu():.2f}")
