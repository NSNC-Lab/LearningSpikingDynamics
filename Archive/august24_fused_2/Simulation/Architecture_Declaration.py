...
def build_network(args, device, params):
    ...
    neuron_init["BookKeeping"]['rate_loss'] = torch.zeros((args['simulation']['batch_size'],len(args['simulation']['cell_targets']),args['simulation']['epochs']), device=device, dtype=torch.float32)
    neuron_init["BookKeeping"]['sse_loss'] = torch.zeros((args['simulation']['batch_size'],len(args['simulation']['cell_targets']),args['simulation']['epochs']), device=device, dtype=torch.float32)
    neuron_init["BookKeeping"]['psth_loss'] = torch.zeros((args['simulation']['batch_size'],len(args['simulation']['cell_targets']),args['simulation']['epochs']), device=device, dtype=torch.float32)
    neuron_init["BookKeeping"]['cv_loss'] = torch.zeros((args['simulation']['batch_size'],len(args['simulation']['cell_targets']),args['simulation']['epochs']), device=device, dtype=torch.float32)
    neuron_init["BookKeeping"]['fused_loss'] = torch.zeros((args['simulation']['batch_size'],len(args['simulation']['cell_targets']),args['simulation']['epochs']), device=device, dtype=torch.float32)

    batch, cells, params_count = args['simulation']['batch_size'], len(args['simulation']['cell_targets']), args['simulation']['num_params']
    neuron_init["CV"] = {
        "q_window": torch.zeros((batch,10,cells,11), device=device),
        "dq_window": torch.zeros((batch,10,cells,11,params_count), device=device),
        "previous_time": torch.full((batch,10,cells),-1.0, device=device),
        "previous_ds": torch.zeros((batch,10,cells,params_count), device=device),
        "n": torch.zeros((batch,cells), device=device),
        "sum": torch.zeros((batch,cells), device=device),
        "sum_sq": torch.zeros((batch,cells), device=device),
        "dsum": torch.zeros((batch,cells,params_count), device=device),
        "dsum_sq": torch.zeros((batch,cells,params_count), device=device),
    }
...
def declare_neuron_properties(args, device, params, this_neuron_init, name, C = 0.1, g_L = 1/200, E_L = -65, t_ref = 0, E_k = -80, tau_ad = 100, g_inc = 0.0003, Itonic = 0 , V_thresh = -47, V_reset = -54, g_postIC = 0.17, E_exc = 0, nSYN = 0.015, noise_E_exc = 0, tauR_N = 0.7, tauD_N = 1.5, noise = 0,output=0, input=0):
    ...
    if output == 1:
        ...
        this_neuron_init["Learnable"][name]["output_ad_trace"] = torch.zeros((args['simulation']['batch_size'],10,len(args['simulation']['cell_targets'])), device=device, dtype=torch.float32)
        ...
        this_neuron_init['Learnable'][name]['abs_ref_accum_rate'] = torch.zeros((params['abs_ref'].shape), device=device, dtype=torch.float32)
        ...
        this_neuron_init['Learnable'][name]['rel_ref_a_accum_rate'] = torch.zeros((params['rel_ref_a'].shape), device=device, dtype=torch.float32)
        ...
        this_neuron_init['Learnable'][name]['rel_ref_b_accum_rate'] = torch.zeros((params['rel_ref_b'].shape), device=device, dtype=torch.float32)
        ...
        this_neuron_init['Learnable'][name]['rel_ref_c_accum_rate'] = torch.zeros((params['rel_ref_c'].shape), device=device, dtype=torch.float32)
