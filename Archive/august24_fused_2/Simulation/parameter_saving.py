...
def save(states,path="simulation_output.mat",checkpoint=False):
    ...
    data = {"output":array(ron["spikes_holder"]).astype(np.uint8),
    ...
    completed = states['epochs_completed']
    data.update({'sse_losses':np.asarray(states['neurons']['BookKeeping']['sse_loss'][...,:completed].cpu(), np.float32), 'psth_losses':np.asarray(states['neurons']['BookKeeping']['psth_loss'][...,:completed].cpu(), np.float32), 'cv_losses':np.asarray(states['neurons']['BookKeeping']['cv_loss'][...,:completed].cpu(), np.float32), 'rate_losses':np.asarray(states['neurons']['BookKeeping']['rate_loss'][...,:completed].cpu(), np.float32), 'fused_losses':np.asarray(states['neurons']['BookKeeping']['fused_loss'][...,:completed].cpu(), np.float32)})
