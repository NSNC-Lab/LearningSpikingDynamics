...
def generate_model_inputs(rates,args,device):
    ...
    onset_probability = torch.clamp(rates['onset_rate']*args['simulation']['dt']/1000, min=0.0, max=1.0)
    onset_spks = 1 * (onset_probability[:,:,:,None] > torch.rand([rates['onset_rate'].shape[0],rates['onset_rate'].shape[1],rates['onset_rate'].shape[2],10], device=device))
    onset_spks = onset_spks.permute(1,3,2,0)
    offset_probability = torch.clamp(rates['offset_rate']*args['simulation']['dt']/1000, min=0.0, max=1.0)
    offset_spks = 1 * (offset_probability[:,:,:,None] > torch.rand([rates['offset_rate'].shape[0],rates['offset_rate'].shape[1],rates['offset_rate'].shape[2],10], device=device))
