...
matr_entries=sum(sto_profs,2)/2;  % here one starts from the spike train order profile!                   
matr = tril(ones(num_trains),-1);
matr(~~matr) = matr_entries';
matr=matr'-matr;               % This is an antisymmetrized version of the spike train order matrix E, it is not actually the SPIKE-order matrix (which is all 0)
